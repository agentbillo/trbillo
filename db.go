...
func CreateBoard(name, description, ownerID string) (*Board, error) {
	...
	queryBoard := `INSERT INTO boards (id, name, description, theme, owner_id, created_at) VALUES (?, ?, ?, ?, ?, ?)`
	if _, err := tx.Exec(queryBoard, b.ID, b.Name, b.Description, b.Theme, b.OwnerID, b.CreatedAt); err != nil {
		return nil, err
	}
...
func GetBoard(id string) (*Board, error) {
	b := &Board{}
	query := `SELECT id, name, description, theme, owner_id, created_at FROM boards WHERE id = ?`
	err := DB.QueryRow(query, id).Scan(&b.ID, &b.Name, &b.Description, &b.Theme, &b.OwnerID, &b.CreatedAt)
	if err != nil {
		return nil, err
	}
	return b, nil
...
func UpdateBoard(id, name, description, theme string) (*Board, error) {
	query := `UPDATE boards SET name = ?, description = ?, theme = ? WHERE id = ?`
	_, err := DB.Exec(query, name, description, theme, id)
	if err != nil {
		return nil, err
	}
	return GetBoard(id)
...
func ListUserBoards(userID string) ([]*Board, error) {
	query := `SELECT b.id, b.name, b.description, b.theme, b.owner_id, b.created_at
	          FROM boards b
	...
	for rows.Next() {
		b := &Board{}
		if err := rows.Scan(&b.ID, &b.Name, &b.Description, &b.Theme, &b.OwnerID, &b.CreatedAt); err != nil {
			return nil, err
		}
